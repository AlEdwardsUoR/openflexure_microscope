...
******************************************************************/
/******************************************************************
* ADE (AKA Al Edwards) introduced offset for dovetail, to try to fit the pi camera module within the larger body          *
*   ALSO reduced length to lens holder dramatically to reduce magnification and increase field of view  
ALSO introduced z-scaling option to further shrink module in z axis*
******************************************************************/
...
module camera_mount_body(
        ...
        dovetail_offset=0 //XXXADE ADDED offest of dovetail, default 0XXX
    ){
    ...
    union(){
        difference(){
            ...
            sequential_hull(){
                ...
                translate([0,0,dt_bottom]) hull(){
                    ...
                    dovetail_mount(shift=dovetail_offset);///ADE ADDED offset of dovetail as value of shift
                ...
                translate([0,0,dt_bottom]) hull(){
                    cylinder(r=bottom_r,h=d);
                    if(fluorescence) cube([1,1,0]*(fl_cube_w+2) + [0,0,d], center=true);
                    dovetail_mount(shift=dovetail_offset-3);//ADE ADDED offset of dovetail to value of shift
                }
                union(){
        ...
        // add the dovetail XXXADE ADDED offest of dovetailXXX
        
        translate([0,objective_clip_y+dovetail_offset,dt_bottom]){
...
module optics_module_trylinder(
    ...
    pedestal_h = 4-2; // extra height on the gripper, to allow it to flex XXADE removed 2 here
    ...
    // having calculated where the lens should go, now make the mount:
    lens_assembly_z = lens_z - pedestal_h-2; //height of lens assembly XXXADE FIDDLED THIS BIT AND CAN MAKE LENS SHORTER 2 BY REDUCING THIS VARIABLE
    ...
    union(){
        ...
        difference(){
            // camera mount with a body that's shorter than the dovetail
            //XXXADE ADDED dovetail offset specified by doveoffset
            camera_mount_body(body_r=lens_assembly_base_r, bottom_r=7, body_top=lens_assembly_z, dt_top=min(lens_assembly_z, z_flexure_spacing),dovetail_offset=doveoffset);
            // camera cut-out and hole for the beam
...
scale([1,1,0.8]){ //XXXXXXSQUASHING z-axis OPTION!!!
difference(){
    if(optics=="pilens"){
        // Optics module for picamera v2 lens, using trylinder
        optics_module_trylinder(
            lens_r = 3, 
            parfocal_distance = 35,//ADE changes this to vary tube length
            lens_h = 2.5,
            doveoffset=5//ADE ADDs value HERE to offset dovetail in y axis here
        );
