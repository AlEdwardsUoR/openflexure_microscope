...
big_stage = true;
xy_lever_ratio=5.8/7.0;//ADE ADDED TO OVERRIDE RATIO, and use TALL version with lower stage; less than 1 good!
sample_z = 46;//ADE ADDED TO OVERRIDE HEIGHT, and use TALL version with lower stage; 42 is too low and parts fall off!
	...
	difference(){
		intersection(){
			union(){
				...
				translate([-clip_outer_w/2,objective_clip_y,0]) cube([clip_outer_w,dy-d,z_flexure_spacing+z_strut_t+4]); //this becomes the dovetail clip ****ADE ADDED 5mm height to dovetail***
			}
...
rotate([0,0,45]) translate([0,0,-2]) cube([17,17,25],centre=true);
}